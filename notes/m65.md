## m65 - the lego keyboard
  ...
  ![m65 rev 11](pics/mlego_m65_rev11_eink.webp){ loading=lazy }

  ![m65 rev 11](pics/mlego_m65_rev11_oled.webp){ loading=lazy }
...
#### rev11
  ...
  ![m65 rev11 xiao rp2040](pics/m65-rev11-3d-front.webp){ loading=lazy }

  ![m65 rev11 xiao rp2040](pics/m65-rev11-3d-back.webp){ loading=lazy }


#### rev10

  ![m65 rev10 xiao rp2040](pics/m65-rev10-3d-front.webp){ loading=lazy }

  ![m65 rev10 xiao rp2040](pics/m65-rev10-3d-back.webp){ loading=lazy }

#### rev9

  the 3d render rev9

  ![m65 rev9 xiao rp2040](pics/m65-rev9-render-front.webp){ loading=lazy }

  ![m65 rev9 xiao rp2040](pics/m65-rev9-render-back.webp){ loading=lazy }

#### rev7-8

  the 3d render rev7/rev8

  ![3d render rev7](pics/m65_3d_rev7.webp){ loading=lazy }

#### rev5/6

  the 3d render rev5/rev6

  ![3d render](pics/m65-rev5-render.webp){ loading=lazy }

  ![3d render back](pics/m65-rev5-render-back.webp){ loading=lazy }

#### rev4

  the 3d rev4

  ![3d render](pics/m65-rev4-render.webp){ loading=lazy }

#### rev2

  version 2

  ![3d render](pics/m65-v2.webp){ loading=lazy }

#### rev1

  the 3d render looks like (version 1)

  ![3d render](pics/m65.webp){ loading=lazy }
...
#### rev7
 ...
 ![schematic ](pics/m65-rev7-schematic.webp){ loading=lazy }

#### rev5

![schematic ](pics/m65-rev5-schematic.webp){ loading=lazy }

#### rev4

![schematic ](pics/m65-rev4-schematic.webp){ loading=lazy }

### pcb

#### rev11

 ![m65 rev11 pcb](pics/m65-rev11-pcb.webp){ loading=lazy }

 ![m65 rev11 pcb](pics/mlego_m65_rev11_pcb_front.webp){ loading=lazy }

 ![m65 rev11 pcb](pics/mlego_m65_rev11_pcb_back.webp){ loading=lazy }


#### rev10

 ![m65 rev10 pcb](pics/m65-rev10-pcb.webp){ loading=lazy }

![m65 rev10 pcb](pics/m65-rev10pcb-front.webp){ loading=lazy }

![m65 rev10 pcb](pics/m65-rev10pcb-back.webp){ loading=lazy }

#### rev9

  the pcb for rev9

  ![m65 rev9 pcb](pics/m65-rev9-pcb.webp){ loading=lazy }

![m65 rev9 pcb](pics/m65-rev9-pcb-front.webp){ loading=lazy }

![m65 rev9 pcb](pics/m65-rev9-pcb-back.webp){ loading=lazy }

#### rev 7/8

   the pcb for rev 7/8

  ![m65 rev7 xiao pcb](pics/m65-rev7-pcb.webp){ loading=lazy }

  ![pcb rev7 back](pics/m65-rev7-pcb-front.webp){ loading=lazy }

  ![pcb rev7 front](pics/m65-rev7-pcb-back.webp){ loading=lazy }

#### rev 5/6

  the pcb for rev5/6

  ![pcb](pics/m65-rev5-pcb.webp){ loading=lazy }

  the real pcb

  ![pcb rev5 front](pics/m65-rev5-pcb-front.webp){ loading=lazy }

  ![pcb rev5 back](pics/m65-rev5-pcb-back.webp){ loading=lazy }

#### rev 4

  version (4) stm32f401 from we act

  ![the pcb](pics/m65-pcb4.webp){ loading=lazy }

  ![the pcb rev4 ](pics/m65-rev4-pcb-front.webp){ loading=lazy }

  ![the pcb rev4 ](pics/m65-rev4-pcb-back.webp){ loading=lazy }

#### rev 3

  stm32f401 from we act

  ![pcb rev3 front](pics/m65-rev3-pcb-front.webp){ loading=lazy }

  ![pcb rev3 back](pics/m65-rev3-pcb-back.webp){ loading=lazy }
...
#### rev 2
  ...
  ![the pcb](pics/m65-pcb-2.webp){ loading=lazy }

#### rev 1

  the pcb (version 1)

  ![the pcb](pics/m65-pcb.webp){ loading=lazy }
...
#### zmk
  ...
  - [firmware/mlego5x13_rev5-rpi_pico-zmk.uf2](https://gitlab.com/m-lego/m65/-/blob/devel/firmware/mlego5x13_rev5-rpi_pico-zmk.uf2)
   ...
   you can get the latest firmware from the [repo actiona](https://github.com/alinelena/mlego-zmk/actions/workflows/build.yml) just look for **Build mlego** workflow and the latest
successful artefacts.

   source for is available

  - [for xiao  and stm32 blackpill](https://github.com/alinelena/mlego-zmk)


  - [rev 4 - stm32f401 blackpill](https://github.com/alinelena/mlego-zmk/tree/main/config/boards/shields/mlego5x13_rev4)
  - [rev 5 - raspberry pico rp2040](https://github.com/alinelena/mlego-zmk/tree/main/config/boards/shields/mlego5x13_rev5)
  - [rev 7 - xiao ble (sense)](https://github.com/alinelena/mlego-zmk/tree/main/config/boards/shields/mlego5x13_rev8)
  - [rev 7 - xiao rp2040](https://github.com/alinelena/mlego-zmk/tree/main/config/boards/shields/mlego5x13_rev7)
...
#### revision 10
  ...
  ![m65 rev 10](pics/m65-rev10-lotr-a.webp){ loading=lazy }

  ![m65 rev 10](pics/m65-rev10-tty-a.webp){ loading=lazy }

  ![m65 rev 10](pics/m65-rev10-tty-b.webp){ loading=lazy }

  ![m65 rev 10](pics/m65-rev10-lotr-b.webp){ loading=lazy }

#### revision 9

  ![m65 rev 9](pics/m65-rev9.webp){ loading=lazy }
...
#### revisions 7
  ...
  ![m65 rev7 xiao rp2040](pics/m65-rev7-xiao-rp2040.webp){ loading=lazy }

  ![m65 rev7 xiao pcb](pics/m65-rev7-nok.webp){ loading=lazy }

#### revision 6

  with rp2040 usb-c, raspberry pico clone

  ![rev5/6](pics/m65-rev5-blank.webp){ loading=lazy }
...
#### revision 4
  ...
  ![mt3 elvish ](pics/m65-elvish.webp){ loading=lazy }

#### revision 3

  ![mt3 elvish ](pics/m65-dwarvish.webp){ loading=lazy }

#### revision 2

  keyboard no 14 (commodore c64 harvest keycaps with mx adapters)

  ![mt3 c64 ](pics/m65_c64.webp){ loading=lazy }

#### revision 1

  keyboard no 2 (mt3 susuwatari, kailh crystal jade)

  ![with susuwatari](pics/m65-susu.webp){ loading=lazy }

####  other

  ![just there](pics/m65-nokey.webp){ loading=lazy }

  keyboard no 1 (mt3 tty bleached, kailh crystal royal)

  ![with dev tty](pics/m65-tty.webp){ loading=lazy }

  ![with dev tty](pics/m65-tty-f.webp){ loading=lazy }

  keyboard no 2 (mt3 susuwatari, kailh crystal jade)

  ![with susuwatari](pics/m65-susu-2.webp){ loading=lazy }

  keyboard no 3 (mt3 tty, kailh crystal royal)

  ![one more /dev/tty](pics/m65-tty-2.webp){ loading=lazy }

  ![one more /dev/tty](pics/m65-home.webp){ loading=lazy }

  ![one more /dev/tty](pics/m65-home-2.webp){ loading=lazy }

  keyboard no 4 (tbd, novelkeys blueberry in process of re-springing)

  ![one more /dev/tty](pics/m65-tty-nkblueberry.webp){ loading=lazy }

  keyboard no 5 (tbd, gateron ink v2 yellow and red)

  ![keycaps to come... dk layout envisaged](pics/m65-gats.webp){ loading=lazy }

  keyboard no 6 (kailh crystal jades with click bar removed... )

  ![keycaps to come... prob mt3 godspeed mixed with tty](pics/m65-jade-lin.webp){ loading=lazy }

  ![keycaps to come... prob mt3 godspeed mixed with tty](pics/m65-jade-lin-2.webp){ loading=lazy }

  ![in /dev/tty with a touch of godspeed and "linear" jades -- clickbars removed](pics/m65-japan.webp){ loading=lazy }

  ![in /dev/tty with a bigger touch of godspeed and "linear" jades -- clickbars removed](pics/m65-japan-2.webp){ loading=lazy }

  keyboard no 7 (gateron whites and /dev/tty)

  ![/dev/tty mt3 ](pics/m65-white-naked.webp){ loading=lazy }

  ![/dev/tty mt3 ](pics/m65-gat-white.webp){ loading=lazy }

  ![/dev/tty and godspeed mods ](pics/m65-g-white.webp){ loading=lazy }

  keyboard no 8 (frankenstein switches linears with mt3 camillo)

  ![mt3 camillo with a touch of godspeed and susuwatari ](pics/m65-olivetti.webp){ loading=lazy }

  keyboard no 9 (mt3 2048)

  ![mt3 2048 with a touch of colour ](pics/m65-2048.webp){ loading=lazy }

  keyboard no 10 (mt3 dasher)

  ![mt3 dasher ](pics/m65-dasher.webp){ loading=lazy }

  keyboard no 11 (mt3 cyber)

  ![mt3 mito cyber ](pics/m65-cyber.webp){ loading=lazy }

  keyboard no 12 (mt3 3277)

  ![mt3 matt3o 3277 ](pics/m65-3277.webp){ loading=lazy }

  keyboard no 13 (mt3 fairlane)

  ![mt3 fairlane ](pics/m65-fair.webp){ loading=lazy }


  keyboard no 15 (mt3 camillo, eva 02)

  ![mt3 camillo ](pics/m65-camillo.webp){ loading=lazy }
...
### keymaps/layout
   ...
   ![layout simplified summary](pics/m65-l1.webp){ loading=lazy }

   ![layout lwr](pics/m65-lwr.webp){ loading=lazy }

   ![layout rse](pics/m65-rse.webp){ loading=lazy }


### pins

##### rev11 seeeduino xiao rp2040

tba

##### rev10 seeeduino xiao rp2040
